...
import { AGENT_ROLES, AgentRoleManager, WORKFLOW_PATTERNS } from '../src/services/agentRoles';
...
class AgentRolesTester {
  ...
  async testAgentRole(agent: any): Promise<void> {
  ...
  generateTestInput(agent: any): any {
  ...
  validateAgentOutputs(agent: any, result: any): void {
  ...
  async testWorkflowPatterns(): Promise<void> {
  ...
  async testWorkflowPattern(patternName: string): Promise<void> {
  ...
  generatePatternTestInput(patternName: string): any {
    switch (patternName) {
  ...
  validatePatternOutputs(patternName: string, result: any): void {
    if (!result.success) return;
  ...
  async testAgentHandoff(test: any): Promise<void> {
    console.log(`🤝 Testing ${test.name}`);
    
    try {
      const steps = test.agents.map((agentId: string) => {
      ...
      console.log(`   🔄 Handoffs: ${result.auditTrail.filter((e: any) => e.action === 'handoff').length}`);
  ...
  async testErrorScenario(test: any): Promise<void> {
    console.log(`⚠️  Testing ${test.name}`);
    
    try {
      const steps = test.agents ? test.agents.map((agentId: string) => {
        const role = AGENT_ROLES[agentId];
        return {
          id: role.id,
          agentType: role.agentType,
  ...
  async testPerformanceScenario(test: any): Promise<void> {
    console.log(`⚡ Testing ${test.name}`);
  ...
  printTestSummary(): void {
    ...
    if (successfulTests > 0) {
      const avgTime = this.testResults
        ...
        .reduce((sum, r) => sum + (r.executionTime || 0), 0) / successfulTests;
